...
<template>
    <view>
        <!-- 时间组件 -->
        <times :dateDetail="dateDetail" @getDate="getDate" @enDate="enDate"/>
        <!-- 档期选择 -->
        <view class="timeBox paddingRL40 ">
            <view class="fontWight paddingB20">热门档期需另付档期费用</view>
            <view class="timeList flex"> 
                <view class="list" v-for="(item,index) in momeyTime" :key="index">
                    <view class="listTime">{{item.tiem}}</view>
                    <view class="fontSize24 marginT10">{{item.text}}</view>
                </view>
            </view>
        </view>
    </view>
